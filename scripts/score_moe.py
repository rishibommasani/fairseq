...
def multi_ref(refs, hypos):
    ...
    # first transpose refs and hypos
    refs = list(zip(*refs))
    hypos = list(zip(*hypos))
    bleus = []
    for hs in hypos:
        bleu = corpus_bleu(hs, refs)
        bleus.append(bleu)
    print('average corpus BLEU: %.2f' % np.mean(bleus))
...
def intra_ref(refs):
    ...
    for rs in refs:
        for i, h in enumerate(rs):
            rest = rs[:i] + rs[i+1:]
            s = [sentence_bleu(h, r) for r in rest]
            j = np.argmax(s)
            _ref.append(rest[j])
            _hypo.append(h)
    print('ref avg oracle BLEU (leave-one-out): %.2f' % corpus_bleu(_hypo, [_ref]))
